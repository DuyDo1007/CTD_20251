...
void compileCondition(void) {
  ...
  if (leftType->typeClass == TP_ARRAY) {
  ...
  if (rightType->typeClass == TP_ARRAY) {
    error(ERR_TYPE_INCONSISTENCY, lookAhead->lineNo, lookAhead->colNo);
  }
...
Type* compileIndexes(Type* arrayType) {
  while (lookAhead->tokenType == SB_LSEL) {
    if (arrayType->typeClass != TP_ARRAY) {
      error(ERR_INVALID_ARRAY, lookAhead->lineNo, lookAhead->colNo);
    }
    eat(SB_LSEL);
    Type* indexType = compileExpression();
    if (indexType->typeClass != TP_INT) {
      error(ERR_INVALID_INDEX, lookAhead->lineNo, lookAhead->colNo);
    }
    eat(SB_RSEL);
