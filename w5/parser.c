...
void compileProgram(void) {
  ...
  Object *progObj = createProgramObject(currentToken->string);
  enterBlock(progObj->progAttrs->scope);
...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      eat(TK_IDENT);
      Object *consObj = createConstantObject(currentToken->string);
      declareObject(consObj);

      eat(SB_EQ);
      consObj->constAttrs->value = compileConstant();
      eat(SB_SEMICOLON);
...
void compileParam(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    paraObj = createParameterObject(currentToken->string, paramKind, symtab->currentScope->owner);
    eat(SB_COLON);
    type = compileBasicType();
    paraObj->paramAttrs->type = type;
    declareObject(paraObj);
    break;
  case KW_VAR:
